...
query ='''
select *
from grp_gdoop_clv_db.cdf_final_scoring
...
process = subprocess.Popen(['hive','-e','"set hive.exec.dynamic.partition.mode=nonstrict; insert overwrite table grp_gdoop_clv_db.cdf_deact_predictions1 partition(ds) select a.*, current_date as ds from grp_gdoop_clv_db.cdf_deact_predictions a where date_sub(current_date,1) <= deact_date sort by deact_date"'], stdout=subprocess.PIPE)
output, error = process.communicate()
...
process = subprocess.Popen(['hive','-e','"set hive.exec.dynamic.partition.mode=nonstrict; insert overwrite table grp_gdoop_clv_db.cdf_deact_forecast1 partition(ds) select a.*, current_date as ds from grp_gdoop_clv_db.cdf_deact_forecast A where date_sub(current_date,1) <= deact_date sort by deact_date"'], stdout=subprocess.PIPE)
output, error = process.communicate()
